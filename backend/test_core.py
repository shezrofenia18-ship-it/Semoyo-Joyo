...
async def main():
    ...
    async with AsyncSessionLocal() as s:
        ...
        order = Order(order_number="MBG-POC-" + str(id(s))[-6:], user_id=user.id, customer_name=user.full_name,
                      ...
                      payment_method="online", payment_channel="qris", payment_payload={"provider": "batpay"})
        s.add(order)
        await s.flush()
        ...
        s.add(PaymentTransaction(order_id=order.id, provider="batpay", method="online",
        ...
        assert o.payment_payload["provider"] == "batpay"
